...
[test_acc2, y2, yhat2] = classifier(num_tree,mti2(feats2,:),labels);
...
final_acc = zeros(length(num_feats),length(num_class));

for j = 1:length(num_class)
    clear labels2 sel_dataset0 sel_dataset1 sel_dataset2
    for i = 1:j+1
        labels2((i-1)*8+1:i*8) = labels((I0(i)-1)*8+1:I0(i)*8);
    ...
    [test_acc0, y0, yhat0] = classifier(num_tree,sel_dataset0,labels2);
    final_acc(1,j) = test_acc0;
    msg = strcat(['Final accuracy of mti0 when # of classes is ',num2str(num_class(j)), ': ',num2str(test_acc0,'%3.1f'),'%.']);
    disp(msg);

    [test_acc1, y1, yhat1] = classifier(num_tree,sel_dataset1,labels2);
    final_acc(2,j) = test_acc1;
    msg = strcat(['Final accuracy of mti1 when # of classes is ',num2str(num_class(j)), ': ',num2str(test_acc1,'%3.1f'),'%.']);
    disp(msg);

    [test_acc2, y2, yhat2] = classifier(num_tree,sel_dataset2,labels2);
    final_acc(3,j) = test_acc2;
    msg = strcat(['Final accuracy of mti2 when # of classes is ',num2str(num_class(j)), ': ',num2str(test_acc2,'%3.1f'),'%.']);
    disp(msg);
...
p3 = plot(num_class, final_acc(3,:),'k-.p', 'LineWidth',6,'markersize',9);
methods = {'No HPF','After HPF','Concatenation'};
legend([p1, p2, p3],{methods{1}, methods{2}, methods{3}});
set(gcf, 'color','w');
